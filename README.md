...
所谓**狄利克雷卷积**是定义在**数论函数**（$\Z_+\to\Complex$的函数）间的一种二元运算，记$f$、$g$为两个数论函数，其狄利克雷卷积记为：

$(f*g)(n):=\sum\limits_{d|n}{f(d)g\left(\frac{n}{d}\right)}$

一些常见的数论函数为：

- 单位函数$\varepsilon(n):=\left\{\begin{matrix}1&,n=1\\0&,n>1\end{matrix}\right.$

- 幂函数$\mathrm{Id}_k(n):=n^k$，当$k=1$时为恒等函数$\mathrm{Id}(n)$，$k=0$时为常值函数$\bold{1}(n)$

- 除数函数$\sigma_k(n):=\sum\limits_{d|n}{d^k}$，当$k=1$时为因数和函数$\sigma(n)$，$k=0$时为因数个数函数$\mathrm{d}(n)$

  - 特殊地，若$n=\prod\limits_{p_i\in\mathbb{P}}{p_i^{c_i}}$则

    - 根据普通生成函数可知：$\sigma(n)=\prod\limits_{p_i\in\mathbb{P}}\left[\sum\limits_{j=0}^{c_i}{p_i^j}\right]$

    - $\mathrm{d}(n)=\prod{(c_i+1)}=\sum\limits_{d|n}\bold1$

- 欧拉函数$\varphi(n):=\sum\limits_{i=1}^{n-1}{\varepsilon\left(\gcd(i, n)\right)}$

- 莫比乌斯函数$\mu(n):=\left\{\begin{matrix}1&,n=1\\-\mu\left(\frac{n}{p}\right)&,n=\prod\limits_{p\in\mathbb{P}}{p}\\0&,otherwise\end{matrix}\right.$

上述函数都是**积性函数**即$\gcd(a,b)=1\Rightarrow f(a)f(b)\equiv f(ab)$

其中单位函数和幂函数是**完全积性函数**，即$\forall a,b~~f(a)f(b)\equiv f(ab)$

狄利克雷卷积保持**积性不变**，即$f$、$g$都是积性函数，则$(f*g)$也是积性质函数

狄利克雷卷积满足**交换律**、**结合律**和对**加法的分配率**

狄利克雷卷积具有**幺元**$\varepsilon(n)$满足$(\varepsilon*f)(n)=\sum\limits_{d|n}{\varepsilon(d)f\left(\frac{n}{d}\right)}=f(n)$

狄利克雷卷积意义下的逆元：

若$\left(f*f^{-1}\right)\equiv\varepsilon$则称$f^{-1}$为$f$的**狄利克雷逆元**，满足：

$$
f^{-1}(n):=
\left\{\begin{matrix}
\frac{1}{f(1)}&,n=1\\
-\frac{1}{f(1)}\sum\limits_{d|n,d\gt1}{f(d)f^{-1}\left(\frac{n}{d}\right)}&,n>1
\end{matrix}\right.
$$

狄利克雷逆元存在的**充要条件**是$f(1)\ne0$，故所有积性函数都是**狄利克雷可逆**的

若$f$、$g$都是狄利克雷可逆的，则$(f*g)^{-1}\equiv (f^{-1}*g^{-1})$

特殊的狄利克雷卷积等式：

- $\left(\mathrm{Id_k}*\bold{1}\right)=\sigma_k$

- $\left(\varphi*\bold{1}\right)(n)=\sum\limits_{d|n}{\varphi(d)}=\mathrm{Id}(n)$

- $(\bold1*\bold1)=\mathrm{d}$

- $\left(\mu*\bold{1}\right)=\varepsilon$即$\mu=\bold{1}^{-1}$

根据上面的等式可以得出其他等式，如：

- $\sigma=(\mathrm{Id}*\bold1)=(\varphi*\bold1*\bold1)=(\varphi*\mathrm{d})$

- $(\mu*\mathrm{d})(n)=\sum\limits_{d|n}{\mu(d)\mathrm{d}\left(\frac{n}{d}\right)}=(\mu*\bold1*\bold1)(n)=\bold1(n)=1$

## 整除分块

整除分块解决这一类问题：$\sum\limits_{i=1}^{n}{\left\lfloor{n\over i}\right\rfloor}$

注意到对于连续的一段$i\to j$，$\left\lfloor{n\over i}\right\rfloor$和$\left\lfloor{n\over j}\right\rfloor$的值是一致的

**结论**是对于$i\in[i,\frac{n}{\left\lfloor{n/i}\right\rfloor}]$都有整除值相同

多维整除分块：解决$\sum\limits_{i=1}^{n}{\left\lfloor{n\over i^k}\right\rfloor}$可以先将$i^k$作为整体得到对应的上界，然后开$k$次根号即$\left\lfloor\sqrt{\frac{n}{\left\lfloor{\frac{n}{i^k}}\right\rfloor}}\right\rfloor$

## 莫比乌斯反演

莫比乌斯反演公式：$g=(f*\bold{1})\Longleftrightarrow f=(g*\mu)$

**莫比乌斯反演的因数形式**：

$g(n)=\mathrm{fac}(f)(n)=\sum\limits_{d|n}{f(d)}\Longleftrightarrow f(n)=\sum\limits_{d|n}{\mu(d)g\left(\frac{n}{d}\right)}$

**莫比乌斯反演的倍数形式**：

$g(n)=\mathrm{mul}(f)(n)=\sum\limits_{n|N}{f(N)}\Longleftrightarrow f(n)=\sum\limits_{n|N}{\mu\left(\frac{N}{n}\right)g(N)}$

**莫比乌斯反演的非卷积形式**：其中$t(n)$为完全积性函数

$g(n)=\sum\limits_{i=1}^{n}{t(i)f\left(\left\lfloor\frac{n}{i}\right\rfloor\right)}\Longleftrightarrow f(n)=\sum\limits_{i=1}^{n}{\mu(i)t(i)g\left(\left\lfloor\frac{n}{i}\right\rfloor\right)}$

gcd卷积，定义为：$h(n):=\sum\limits_{\gcd(x,y)=n}{f(x)g(y)}$

通过莫比乌斯反演求解：

$$
\begin{align*}
\mathrm{mul}(h)(n)&=\sum\limits_{n|N}h(N)\\
&=\sum\limits_{n|x\wedge n|y}{f(x)g(y)}\\
&=\left(\sum\limits_{n|N}{f(N)}\right)\left(\sum\limits_{n|N}{g(N)}\right)\\
&=\left(\mathrm{mul}(f)\cdot\mathrm{mul}(g)\right)(n)\\
\Longrightarrow h(n)&=\sum\limits_{n|N}{\mu\left(\frac{N}{n}\right)\mathrm{mul}(h)(N)}\\
&=\sum\limits_{n|N}{\mu\left(\frac{N}{n}\right)\left(\mathrm{mul}(f)\cdot\mathrm{mul}(g)\right)(N)}
\end{align*}
$$

最大公约数等价：$d|\gcd(x_1,x_2,\dots,x_n)\Longleftrightarrow \bigwedge\limits_{i=1}^{n}{d|x_i}$

最小公倍数等价：$d|\mathrm{lcm}(x_1,x_2,\dots,x_n)\Longleftrightarrow \bigvee\limits_{i=1}^{n}{d|x_i}$

因数求和转倍数求和转整除分块：

$$
\sum_n^N\sum\limits_{d|n}F=\sum_d^N\sum\limits_{d|n}F\\
\sum\limits_{i=1}^{n}\sum\limits_{d|i}{F(d,i)}=\sum\limits_{d=1}^{n}\sum\limits_{i=1}^{\left\lfloor{n\over d}\right\rfloor}{F(d,id)}\\
\sum\limits_{i=1}^{n}\sum\limits_{j=1}^{m}\sum\limits_{d|\gcd(i,j)}^{}{F(d,i,j)}=\sum\limits_{d=1}^{\min(n,m)}\sum\limits_{i=1}^{\left\lfloor{n\over d}\right\rfloor}\sum\limits_{j=1}^{\left\lfloor{m\over d}\right\rfloor}{F(d,id,jd)}\\
\dots
$$

## 杜教筛：

对于**数论函数**$f$，想要求得$S(n)=\sum\limits_{i=1}^{n}{f(i)}$，若能找到函数$g$且能容易求得$\sum\limits_{i=1}^{n}{(g*f)(i)}$和$\sum\limits_{i=1}^{n}{g(i)}$
...
=&\sum_{y=1}^{n}\sum_{x=1}^{\left\lfloor{n\over y}\right\rfloor}{f(x)g(y)}\\
=&\sum_{y=1}^{n}g(y)\sum_{x=1}^{\left\lfloor{n\over y}\right\rfloor}{f(x)}\\
=&\sum_{y=1}^{n}g(y)S\left(\left\lfloor{n/y}\right\rfloor\right)\\
=&g(1)S(n)+\sum_{y=2}^{n}g(y)S\left(\left\lfloor{n/y}\right\rfloor\right)
\end{align*}
$$

于是乎：$S(n)=\dfrac{1}{g(1)}\left({\sum\limits_{i=1}^{n}{(g*f)(i)}-\sum_{y=2}^{n}g(y)S\left(\left\lfloor{n/y}\right\rfloor\right)}\right)$

后者可以由整除分块递归求解，复杂度$O(n^{4\over 5})$

若使用线性筛筛出$S(n)$的前$n^{2\over3}$项值，则总复杂度可以降为$O(n^{2\over3})$，此外，还可以开一个哈希表对大于$n^{2\over3}$的数也进行记忆化！

常见的杜教筛公式：

- $S(n)=\sum\limits_{i=1}^{n}{\mu(i)}$则：$S(n)=1-\sum_{y=2}^{n}S\left(\left\lfloor{n/y}\right\rfloor\right)$

- $S(n)=\sum\limits_{i=1}^{n}{\varphi(i)}$则：$S(n)=\frac{n(n+1)}{2}-\sum_{y=2}^{n}S\left(\left\lfloor{n/y}\right\rfloor\right)$
